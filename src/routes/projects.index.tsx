      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "clamp(36px,5vw,56px) clamp(18px,5vw,40px) clamp(56px,8vw,88px)", display: "grid", gap: 22 }}>
        {PROJECT_SUMMARIES.map((p, idx) => (
          <Link
            ...
            style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${p.color}`, overflow: "hidden", display: "grid", gridTemplateColumns: "1fr", borderRadius: 2 }}
          >
            <div className="proj-card-img project-img" style={{ position: "relative", overflow: "hidden", minHeight: 220 }}>
              ...
              <div style={{ position: "absolute", top: 14, right: 14, background: `${p.color}22`, border: `1px solid ${p.color}55`, padding: "4px 10px", fontSize: 9, letterSpacing: 1.5, color: p.color }}>Available</div>
            </div>
            <div style={{ padding: "clamp(22px,3vw,40px)" }}>
              <div style={{ fontSize: 9, letterSpacing: 2, color: p.color, border: `1px solid ${p.color}44`, display: "inline-block", padding: "4px 11px", marginBottom: 14, textTransform: "uppercase" }}>{p.cat}</div>
              <h2 className="serif" style={{ fontWeight: 400, marginBottom: 8, fontSize: "clamp(30px,3.5vw,44px)" }}>{p.name}</h2>
              <div style={{ fontSize: 10, color: C.gold, letterSpacing: 1.5, marginBottom: 14 }}>📍 {p.loc}</div>
              <p style={{ fontSize: 13, color: C.muted, lineHeight: 1.8, marginBottom: 18 }}>{p.desc}</p>
              <div style={{ display: "flex", gap: 22, marginBottom: 20 }}>
                <div>
                  <div style={{ fontSize: 9, color: C.muted, marginBottom: 3, letterSpacing: 1 }}>PLOTS</div>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>{p.plots}</div>
                </div>
                <div>
                  <div style={{ fontSize: 9, color: C.muted, marginBottom: 3, letterSpacing: 1 }}>SIZE</div>
...
export const Route = createFileRoute("/projects/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse Pride, Aerox, Regalia 3 & Paradise — residential, commercial & township plots in Dholera SIR. All NA/NOC certified with 100% title clear. Sizes from 144 to 500 sq.yd." },
